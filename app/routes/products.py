...
def create_product(
    ...
    print("[DEBUG] create_product endpoint called")
    """建立新商品（需要管理員權限）"""
    # 檢查 SKU 是否重複
...
def update_product(
    ...
    if "name" in update_data:
        ...
        slug_exists = (
            db.query(Product)
            .filter(Product.slug == product.slug, Product.id != product_id)
            .first()
        )
        if slug_exists:
            import time
...
def delete_product(
    ...
    if product.order_items:
        raise HTTPException(status_code=400, detail="無法刪除：此商品已有相關訂單")
